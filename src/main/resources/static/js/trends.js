...
function getRandomColorArray () {
    return [randClr(), randClr(), randClr(), randClr(),randClr(), randClr(), randClr(), randClr(), randClr(), randClr()];
}
...
$(function () {
    ...
    $.getJSON("http://localhost:8080/trends", {
        ...
        if(results.length) {
            const names = results.map(function(result) {
                return result['person'];
            });
            const hits = results.map(function(result) {
                return result['hits'];
            });
                ...
                options: {
                    ...
                    'onClick' : function (event, item) {
                        if(item.length) {
                            ...
                            const itemClicked = item[0]._model.label;
                            inputField.val(itemClicked);
