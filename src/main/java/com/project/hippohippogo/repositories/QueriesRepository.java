...
public interface QueriesRepository extends JpaRepository<Query, QueryId> {
    @org.springframework.data.jpa.repository.Query(value = "SELECT query FROM queries WHERE user_ip = ?1 AND query REGEXP ?2 ORDER BY hits DESC LIMIT ?3", nativeQuery = true)
